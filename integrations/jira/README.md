...
python -m agents.orchestrator --jql "project = MYPROJ AND sprint in openSprints()"
```

Then update `RequirementsAgent.fetch_tickets()` in `agents/requirements_agent.py` to pass the `Authorization` header from `JIRA_TOKEN`.
